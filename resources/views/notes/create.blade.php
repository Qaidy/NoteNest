<x-app-layout>
    <div class="max-w-4xl mx-auto">
        ...
        <div class="flex items-center gap-4 mb-8">
            ...
            <div>
                <h1 class="font-bold text-2xl text-gray-900 dark:text-white leading-tight">Create New Note</h1>
            </div>
        </div>

        <!-- Form Card -->
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700/60 overflow-hidden">
            <form action="{{ route('notes.store') }}" method="POST" class="p-6 sm:p-8">
                @csrf
                ...
                <div class="mb-8">
                    ...
                    <textarea name="content" id="content" rows="12" 
                              ...
                              placeholder="Start typing your note here..." required>{{ old('content') }}</textarea>
                    @error('content')
                        <p class="mt-2 text-sm text-red-500 font-medium">{{ $message }}</p>
                    @enderror
                </div>
